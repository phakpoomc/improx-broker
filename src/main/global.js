...
export async function loadOverview(isClear) {
    ...
    const group = await db.group.findAll({
        where: {
            type: {
                [Op.or]: [{ [Op.like]: 'overview-%' }, { [Op.like]: 'go-%' }]
            }
        }
    })
    ...
    for (const g of group) {
        if (group !== null) {
            ...
            if (
                g.type.startsWith('overview-kWh') ||
                g.type == 'overview-incomming' ||
                g.type?.startsWith('go-value')
            ) {
            ...
            if (
                g.type.startsWith('overview-mold') ||
                g.type.startsWith('overview-furnace') ||
                g.type.startsWith('overview-air-comp')
            ) {
            ...
            if (members !== null) {
                for (const m of members) {
                    ...
                    if (g.type == 'overview-incomming' || g.type?.startsWith('go-value')) {
                        ...
                        overview_store.monthly_kwh[g.type].prevValue[key] = 0
                        fetch_keys.push(key)
                        const dat = await redisClient.get(
                            `${m.SerialNo + '%' + String(m.ModbusID - 1)}.dat`
                        )
                        
                        if (!dat) {
                            await redisClient.set( `${m.SerialNo + '%' + String(m.ModbusID - 1)}.dat`,JSON.stringify({ data: [] }));
                        }
                        let ov_json = null
                        try {
                            ov_json = JSON.parse(dat)
                        } catch (err) {}
                        if (ov_json && ov_json['data'].length > 0) {
                            overview_store[m.SerialNo + '%' + String(m.ModbusID - 1)] =
                                [ov_json['data'][ov_json['data'].length - 1]]
                        } else {
                            overview_store[m.SerialNo + '%' + String(m.ModbusID - 1)] = []
                        }
                    }
                    ...
                    if (g.type.startsWith('go-graph-extend'))
                        overview_store['multiplier'][g.type + g.name + '-' + key] = m.multiplier
    ...
    const startTime = new Date(Date.UTC(newMonth.getFullYear(), newMonth.getMonth(), 1, 0, 0, 0))
